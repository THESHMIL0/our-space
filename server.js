...
function initMemory() { 
    const e = ['🍎','🍎','🍌','🍌','🍇','🍇','🍉','🍉','🍓','🍓','🍒','🍒','🍍','🍍','🥝','🥝']; 
    memoryBoard = e.sort(() => Math.random() - 0.5); memoryFlipped = []; memoryMatched = []; 
}
initMemory();
...
io.on('connection', (socket) => {
    ...
    socket.on('mem flip', (idx) => {
        ...
        if(memoryFlipped.length < 2) {
            ...
            io.emit('memory update', { board: memoryBoard, flipped: memoryFlipped, matched: memoryMatched });
            if(memoryFlipped.length === 2) {
                let i1 = memoryFlipped[0], i2 = memoryFlipped[1];
                if(memoryBoard[i1] === memoryBoard[i2]) {
                    memoryMatched.push(i1, i2); memoryFlipped = [];
    ...
    socket.on('qd start', () => {
        ...
        qdTimeout = setTimeout(() => { qdState = 'go'; io.emit('qd update', qdState); }, Math.random() * 3000 + 2000);
    });
    socket.on('qd tap', () => {
        if(qdState === 'go') { qdState = 'win'; io.emit('qd update', qdState); setTimeout(() => { qdState = 'idle'; io.emit('qd update', qdState); }, 3000); } 
